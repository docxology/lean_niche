...
class LeanRunner:
    ...
    def run_lean_code(self, code: str, imports: List[str] = None) -> Dict[str, Any]:
        ...
        # If the code or imports reference local Lean modules (e.g. LeanNiche.*),
        # attempt to build the Lean project with `lake` so `.olean` artifacts exist
        # and imports resolve. This makes invoking Lean from Python more robust.
        try:
            needs_build = any('LeanNiche' in imp for imp in imports) or ('LeanNiche' in code)
        except Exception:
            needs_build = False

        if needs_build:
            # Prefer using lake to build the project if available
            try:
                from shutil import which
                lake_exe = which('lake')
                if lake_exe:
                    self.logger.info('Detected LeanNiche imports; running `lake build` to produce .olean files')
                    subprocess.run([lake_exe, 'build'], check=True, capture_output=True, text=True)
                else:
                    # Fallback: add src/lean to LEAN_PATH so lean can find modules (best-effort)
                    project_root = Path(__file__).parent.parent.parent
                    src_lean = str(project_root / 'src' / 'lean')
                    cur = os.environ.get('LEAN_PATH', '')
                    if src_lean not in cur:
                        os.environ['LEAN_PATH'] = src_lean + os.pathsep + cur if cur else src_lean
                        self.logger.info(f"Set LEAN_PATH to include project Lean sources: {src_lean}")
            except subprocess.CalledProcessError as e:
                self.logger.warning(f"`lake build` failed: {e}; Lean imports may not resolve")
            except Exception as e:
                self.logger.debug(f"Build helper skipped: {e}")
        ...
        try:
            ...
            # Parse results from runtime output
            parsed_result = self._parse_lean_output(result)

            # Also extract declarations/theorems directly from the submitted code
            try:
                code_extracted = self.extract_mathematical_results(code)
                # Merge extracted theorems into parsed_result in a compatible format
                for th in code_extracted.get('theorems', []):
                    parsed_result.setdefault('theorems_proven', []).append({
                        'type': 'theorem',
                        'name': th.get('name'),
                        'statement': th.get('statement')
                    })
                for df in code_extracted.get('definitions', []):
                    parsed_result.setdefault('definitions_created', []).append({
                        'type': 'def',
                        'name': df.get('name'),
                        'body': df.get('body')
                    })
                for comp in code_extracted.get('computations', []):
                    parsed_result.setdefault('computation_results', []).append(comp)
            except Exception as e:
                self.logger.debug(f"Failed to extract declarations from code: {e}")

            # If any errors were detected in parsing, mark success accordingly
            success_flag = parsed_result.get('verification_status', {}).get('compilation_successful', True)

            self.logger.info(f"Executed Lean code. success={success_flag}; parsed_result summary: { {k: len(v) for k,v in parsed_result.items() if isinstance(v, list)} }")
    ...
    def _execute_lean(self, lean_file: Path) -> Dict[str, Any]:
        ...
        # Run Lean from the project root and ensure LEAN_PATH includes local src/lean
        project_root = Path(__file__).parent.parent.parent
        env = os.environ.copy()
        src_lean = str(project_root / 'src' / 'lean')
        lean_path_env = env.get('LEAN_PATH', '')
        if src_lean not in lean_path_env:
            env['LEAN_PATH'] = src_lean + os.pathsep + lean_path_env if lean_path_env else src_lean
